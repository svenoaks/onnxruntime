...
Status T5DecoderSubgraph::CreateInitialFeeds(
  ...
  if (!use_sequence_as_input_ids_) { // use next tokens for input_ids
  ...
  auto add_decoder_feeds = [&](OrtValue& encoder_output, bool is_dynamic_kv_cache) -> int {
    OrtValue expanded;
  ...
  if (has_hidden_state_) {
    add_decoder_feeds(encoder_fetches[1], false);
  }

  // New format of encoder has only cross outputs.
  if (encoder_fetches.size() == 2 * num_layers) {
    for (int i = 0; i < 2 * num_layers; i++) {
        // cross shape is (batch_size, num_heads, encode_sequence_length, head_size)
        const TensorShape& shape = encoder_fetches[0].Get<Tensor>().Shape();
        ORT_ENFORCE(shape.NumDimensions() == 4);

        // Shape for kv cache: (batch_size * num_beams, num_heads, max_seq_len, head_size)
        int64_t dims[4] = {0};
        shape.CopyDims(dims, shape.NumDimensions());
        dims[0] *= num_beams;
        dims[2] = past_present_share_buffer_max_seq_len;
        TensorShape expanded_shape(&dims[0], shape.NumDimensions());

        MLDataType element_type = encoder_fetches[0].Get<Tensor>().DataType();
        OrtValue past_kv_cache;
        Tensor::InitOrtValue(element_type, expanded_shape, allocator, past_kv_cache);
        decoder_feeds.push_back(expanded);
    }

    // Add cross inputs from encoder output.
    for (size_t j = 0; j < encoder_fetches.size(); j++) {
      add_decoder_feeds(encoder_fetches[j], false);
    }
  } else {
    for (size_t j = 1 + has_hidden_state_; j < encoder_fetches.size(); j++) {
        ...
        add_decoder_feeds(encoder_fetches[j], is_dynamic_kv_cache);
    }
  }
