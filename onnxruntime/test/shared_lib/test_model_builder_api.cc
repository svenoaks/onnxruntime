...
Ort::Session CreateSession(Ort::Env& env,
                           ModelEditorAPI::Model& graph_api_model,
  ...
  graph_api_model = ModelEditorAPI::Model(nullptr);
...
TEST(ModelEditorAPITest, Basic_CApi) {
  ...
  auto run_test = [&](bool use_constant_node) -> void {
    ...
    ModelEditorAPI::Model cxx_model(model);
...
TEST(ModelEditorAPITest, Basic_CxxApi) {
  ...
  Ort::ModelEditorAPI::Graph graph;
  ...
  std::vector<ModelEditorAPI::ValueInfo> graph_inputs;
  std::vector<ModelEditorAPI::ValueInfo> graph_outputs;
  ...
  ModelEditorAPI::Node node("Gemm", onnxruntime::kOnnxDomain, "Gemm1", {"X", "Y"}, {"Z"}, attributes);
  ...
  std::vector<ModelEditorAPI::Model::DomainOpsetPair> opsets{{onnxruntime::kOnnxDomain, 18}};
  ModelEditorAPI::Model model(opsets);
...
TEST(ModelEditorAPITest, BasicModelEdit_CxxApi) {
  ...
  std::vector<ModelEditorAPI::Model::DomainOpsetPair> opsets;  // no additional opsets required
  ModelEditorAPI::Model model(opsets);

  std::vector<ModelEditorAPI::ValueInfo> graph_inputs = session.GetInputs();
  ...
  ModelEditorAPI::Node node("Cast", onnxruntime::kOnnxDomain, new_input_name, {"Int64Input"},
  ...
  TensorTypeAndShapeInfo input_tensor_info(ONNXTensorElementDataType::ONNX_TENSOR_ELEMENT_DATA_TYPE_INT64,
                                           input_shape);
  auto input_type_info = TypeInfo::CreateTensorInfo(input_tensor_info.GetConst());
  graph_inputs[0] = ModelEditorAPI::ValueInfo(new_input_name, input_type_info.GetConst());

  ModelEditorAPI::Graph graph;  // new info to augment the model with
...
TEST(ModelEditorAPITest, CreateInvalidModel_NoOpsets) {
  ...
  ModelEditorAPI::Node node("Add", onnxruntime::kOnnxDomain, "Add1", {"X", "X"}, {"Z"});

  graph.AddNode(node);

  std::vector<ModelEditorAPI::Model::DomainOpsetPair> opsets;
  ModelEditorAPI::Model model(opsets);
  model.AddGraph(graph);
...
TEST(ModelEditorAPITest, CreateInvalidModel_MissingValue) {
  ...
  ModelEditorAPI::Node node("Add", onnxruntime::kOnnxDomain, "Add1", {"X", "missing"}, {"Z"});
  graph.AddNode(node);
...
TEST(ModelEditorAPITest, InvalidModelEdit) {
  ...
  const auto edit_model = [](bool invalid_domain) {
    ...
    ModelEditorAPI::Node node("Cast", domain, "NewInputNode", {new_input_name},
                               // the existing node will now consume the output from the Cast instead of a graph input
                               {graph_inputs[0].Name()},
                               attributes);
    graph.AddNode(node);
