...
template <typename T>
std::vector<ModelEditorAPI::ValueInfo> ConstSessionImpl<T>::GetInputs() const {
  const std::vector<std::string> input_names = GetInputNames();

  std::vector<ModelEditorAPI::ValueInfo> inputs;
  inputs.reserve(input_names.size());

  for (size_t i = 0; i < input_names.size(); ++i) {
    auto type_info = GetInputTypeInfo(i);
    inputs.emplace_back(ModelEditorAPI::ValueInfo{input_names[i], type_info.GetConst()});
  }
...
template <typename T>
std::vector<ModelEditorAPI::ValueInfo> ConstSessionImpl<T>::GetOutputs() const {
  const std::vector<std::string> output_names = GetOutputNames();

  std::vector<ModelEditorAPI::ValueInfo> outputs;
  outputs.reserve(output_names.size());

  for (size_t i = 0; i < output_names.size(); ++i) {
    auto type_info = GetOutputTypeInfo(i);
    outputs.emplace_back(ModelEditorAPI::ValueInfo{output_names[i], type_info.GetConst()});
  }
...
template <typename T>
inline void SessionImpl<T>::FinalizeModelEditorSession(const ModelEditorAPI::Model& model, const SessionOptions& options,
...
inline Session::Session(const Env& env, const ModelEditorAPI::Model& model, const SessionOptions& options) {
...
void FinalizeModelEditorSession(const ModelEditorAPI::Model& model, const SessionOptions& options,
                                OrtPrepackedWeightsContainer* prepacked_weights_container);
